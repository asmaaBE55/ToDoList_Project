...
// 1. IMPORTIAMO jspdf CORRETTAMENTE IN CIMA AL FILE
import { jsPDF } from "jspdf";

const _0x4a2f = {
  0: ["Meditazione mattutina 🧘", "Revisione settimana", "Chiamata famiglia", "Passeggiata nel parco"],
  1: ["Pianificazione settimanale 📋", "Rispondere alle email", "Workout mattutino", "Review obiettivi"],
  2: ["Meeting di lavoro 💼", "Studiare nuovo argomento", "Pausa relax pomeriggio", "Lettura serale"],
  3: ["Sprint review progetto", "Sessione brainstorming 💡", "Palestra", "Meditazione sera"],
  4: ["Deep work session 🎯", "Call con cliente", "Aggiornamento portfolio", "Yoga"],
  5: ["Progetti personali 🚀", "Pulizia spazio lavoro", "Esplorare nuova skill", "Film serale"],
  6: ["Recupero e ricarica 🌟", "Meal prep settimanale", "Lettura libro", "Passeggiata natura"]
};
const _0xm3 = ["Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre"];
const _0xd7 = ["Dom", "Lun", "Mar", "Mer", "Gio", "Ven", "Sab"];
const _0xf1 = (y, m) => new Date(y, m + 1, 0).getDate();
const _0xf2 = (y, m) => new Date(y, m, 1).getDay();
let _0xc9 = 1e3;
const _0xnid = () => ++_0xc9;
...
export default function _0xApp() {
  ...
  useEffect(() => {
    const _v = localStorage.getItem("ai-todo-tasks-light");
    if (_v) _s0xTk(JSON.parse(_v));
  }, []);

  useEffect(() => {
    localStorage.setItem("ai-todo-tasks-light", JSON.stringify(_0xTk));
  }, [_0xTk]);
  ...
  useEffect(() => {
    _s0xCf(false);
    _s0xPR(false);
    if (!_0xTk[_0xDK]) _0xGA();
  }, [_0xDK]);

  const _0xGA = useCallback(() => {
    _s0xAL(true);
    setTimeout(() => {
      const _dw = new Date(_0xY, _0xM, _0xD).getDay();
      const _sg = _0x4a2f[_dw] || _0x4a2f[1];
      const _at = _sg.map(_t => ({ id: _0xnid(), text: _t, done: false, isAi: true }));
      _s0xTk(_p => ({ ..._p, [_0xDK]: _at }));
      _s0xAL(false);
    }, 900);
  }, [_0xDK, _0xY, _0xM, _0xD]);

  const _0xAT = () => {
    if (!_0xNT.trim()) return;
    _s0xTk(_p => ({ ..._p, [_0xDK]: [...(_p[_0xDK] || []), { id: _0xnid(), text: _0xNT.trim(), done: false, isAi: false }] }));
    _0xNT("");
    _s0xCf(false);
    _s0xPR(false);
  };
  ...
  const _0xSV = _id => {
    if (!_0xET.trim()) return;
    _s0xTk(_p => ({ ..._p, [_0xDK]: (_p[_0xDK] || []).map(_t => _t.id === _id ? { ..._t, text: _0xET.trim() } : _t) }));
    _s0xEI(null);
    _s0xET("");
  };
  const _0xCL = () => { _s0xCf(true); _s0xPR(true); _s0xCC(true); setTimeout(() => _s0xCC(false), 3000); };

  // 2. FUNZIONE PDF AGGIORNATA E CORRETTA
  const _0xPDF = () => {
    // Non serve più il caricamento dinamico "await import", usiamo jsPDF direttamente
    ...
    // Salva il PDF cambiando i caratteri non ammessi nei nomi dei file
  ...
  const _0xPM = () => { if (_0xM === 0) { _s0xM(11); _s0xY(_y => _y - 1); } else _ss0xM(_m => _m - 1); _s0xD(1); };
